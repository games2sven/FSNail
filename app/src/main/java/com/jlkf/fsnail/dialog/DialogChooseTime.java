...
import com.jlkf.fsnail.widget.YbTimeSelectorWheelView;
...
public class DialogChooseTime {
    ...
    YbTimeSelectorWheelView pdwvDateSelectorWheelView;

    public DialogChooseTime(Context con, int  width , Dialogcallback callback) {
        this.context = con;
        this.dialogcallback = callback;
        dialog = new AlertDialog.Builder(con).create();
        dialog.show();
        dialog.getWindow().setContentView(R.layout.dialog_picktime);
...
//        pdwvDateSelectorWheelView.setCurrentDay(c.get(Calendar.DAY_OF_MONTH) + "");
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.tv_cancle:
                ...
                break;
            case R.id.tv_sure:
                dialogcallback.pickWeightResult(pdwvDateSelectorWheelView.getSelectedTime());
                dismiss();
                break;
        }
    }
    ...
    public interface Dialogcallback {
        void pickWeightResult(String time);
    }
