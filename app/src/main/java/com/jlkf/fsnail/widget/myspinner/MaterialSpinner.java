...
public class MaterialSpinner extends TextView {
  ...
  private void init(Context context, AttributeSet attrs) {
    ...
    boolean rtl = MaterialUtils.isRtl(context);
    ...
    try {
      ...
      arrowColorDisabled = MaterialUtils.lighter(arrowColor, 0.8f);
    } finally {
      ta.recycle();
    }
...
//    }
    ...
    if (!hideArrow) {
      arrowDrawable = MaterialUtils.getDrawable(context, R.drawable.ms__arrow).mutate();
...
//        }
//      }
      Log.i("Sven","longestItem:"+longestItem+"currentText :"+currentText);
      setText(longestItem);
  ...
  public void setBackgroundColor(int color) {
    ...
    if (background instanceof StateListDrawable) { // pre-L
      try {
        ...
        int[] colors = { MaterialUtils.darker(color, 0.85f), color };
